    ...
    if not indice:
        ...
        for col_num, value in enumerate(df.columns.values):
            arquivo.sheets[nome].write(
                0, col_num + len(df.index.names), value, formato_cabecalho
            )
...
def consolidar_pd(pasta):
    ...
    try:
        ...
        if not com.estr_compativel(df_com.columns):
            ...
            mostra_erro(
                "O arquivo " + arq + " contém colunas incompatíveis: "
            )
...
def criarArquivoMacrosGrafo(
    ...
    for row in dfComunicacoes.itertuples(index=False):
        ...
        item = {
            "id": "COM_" + str(int(row.Indexador)),
            "tipo": "ENT",
            "sexo": 0,
            "label": "COAF R$ {:,}".format((row.CampoA))
            .replace(",", "X")
            .replace(".", ",")
            .replace("X", "."),
            "camada": 0,
    ...
    for row in dfEnvolvidos.itertuples(index=False):
        ...
        if cpfcnpj not in cs:
            ...
            item = {
                "id": cpfcnpj
            }  # passando somente o id, na hora de importar o navegador vai pedir os dados dos nós para o servidor
            if len(cpfcnpj) == 11:
                item["tipo"] = "PF"
            else:
                item["tipo"] = "PJ"
